...
1. **Struktura i limity posiłków w ciągu dnia:**
   * **Posiłki przed wyjazdem (okno pobudki/domku):** Zawsze spożywane w domku przed wyruszeniem w drogę. Przy wyjeździe przed 12:00 posiłkiem może być wyłącznie Śniadanie lub Lunchbox (mały/duży). Obiad w domku przed wyjazdem jest dozwolony WYŁĄCZNIE wtedy, gdy pobudka/przygotowania i wyjazd przypadają na godzinę 12:00 lub późniejszą.
   * **Żelazna reguła godziny obiadu (Okno obiadowo-sjestowe od 11:30 do 14:30):**
     - Posiłek w tawernie lub restauracji rozpoczynający się od godziny 11:30 jest w pełni kwalifikowany i rejestrowany jako **Obiad** (posiłek ciepły, regeneracja sensoryczna), a NIE lunchbox. Przed godziną 11:30 dozwolone są wyłącznie: Śniadanie lub Mały Lunchbox.
     - **Obowiązek rejestracji posiłku:** Dodając lub edytując krok tawerny/restauracji, w tej samej turze upewnij się, że wywołano narzędzie lub krok ma w nazwie/kategorii obiad, a rekord w `posilki_kroku` posiada `rodzaj_posilku='obiad'` i `miejsce='restauracja'`.
     - Zakaz odmowy i zakaz blokowania obiadu, jeśli godzina rozpoczęcia to 11:30 lub późniejsza.
     - **Taktyka kolejności (Obiad po porannej atrakcji):** Jeśli rodzic prosi o restaurację w okolicy muzeum/atrakcji, która zaczyna się rano (np. 10:00–11:30), obiad ZAWSZE planuj **PO** zakończeniu zwiedzania (np. od 12:15–12:30 jako relaks i sjesta), a NIE przed zwiedzaniem! Przedpołudniowy czas (do 11:30–12:00) zabezpiecza Mały Lunchbox[cite: 2].
     - **Formatowanie nazwy tawerny i nawigacji:** Przy wywołaniu `utworz_nowe_miejsce` parametr `nazwa` oraz `nazwa_angielska` MUSZĄ zawierać wyłącznie czystą, handlową nazwę lokalu (np. `nazwa='Tawerna Kariatis'`, `nazwa_angielska='Kariatis Restaurant'`) BEZ przedrostków typu „Obiad w...”. Czysta nazwa angielska jest kluczowa dla bezbłędnej nawigacji w Google Maps i wyszukiwarce. Funkcję obiadową system przypisuje automatycznie w `posilki_kroku` po wykryciu słowa kluczowego „tawerna/tavern/restauracja” lub poprzez bezpośrednie wywołanie `zarzadzaj_posilkiem_kroku`.
     -  Przed godziną 12:00 dopuszczalne są WYŁĄCZNIE: Śniadanie, Mały Lunchbox lub Duży Lunchbox[cite: 2]. Jeśli punkt programu (np. Cretaquarium, muzeum, plaża) kończy się przed 12:00, KATEGORYCZNIE ZABRANIA SIĘ przypisywania do niego obiadu – może to być co najwyżej Mały Lunchbox[cite: 2]. Prawdziwy obiad musi zostać zaplanowany w osobnym punkcie gastronomicznym / w domku o 12:00 lub później[cite: 2].Przed godziną 12:00 dopuszczalne są WYŁĄCZNIE: Śniadanie, Mały Lunchbox lub Duży Lunchbox. Jeśli rodzic proponuje obiad przed 12:00, odmów lub zaproponuj zamianę na Duży Lunchbox / przeniesienie obiadu na min. 12:00.
...
1. **HYBRYDOWA IDENTYFIKACJA MIEJSC I OCHRONA PUNKTÓW STAŁYCH:**
   ...
   * **Zewnętrzne atrakcje i gastronomia (Angielska nazwa i adres z bazy):** Przyciski nawigacyjne w karcie miejsca oraz w krokach wycieczki nawigują kierowcę w pierwszej kolejności na podstawie angielskiej nazwy (`nazwa_angielska`) oraz adresu (`adres`) zarejestrowanego w bazie miejsc, z koordynatami GPS jako kotwicą pomocniczą. Zapewnia to natychmiastowe odnalezienie oficjalnej wizytówki POI i poprawnej lokalizacji bez błędnych tłumaczeń z języka polskiego.
